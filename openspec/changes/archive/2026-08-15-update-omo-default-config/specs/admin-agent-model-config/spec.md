...
- **WHEN** an authenticated admin requests the agent model list and `~/.omo/omo.jsonc` contains `agents.plan.models` while the managed opencode server reports `plan` running `opencode-go/kimi-k3`
- **THEN** the response contains a `plan` entry whose `configured` array equals the file's model chain, whose `resolved` equals `{modelID: "kimi-k3", providerID: "opencode-go"}`, and whose source is `configured`

#### Scenario: Agent without configuration reports resolved plugin model
- **WHEN** an agent (e.g. `oracle`) has no model configuration in `~/.omo/omo.jsonc`
...
- **WHEN** an agent entry in `~/.omo/omo.jsonc` fails the pinned OMO schema (e.g. contains a `permission` key unrecognized by 4.19.4)
- **THEN** the response reports the configuration as invalid, and the system does not claim the agent's configured models are effective

### Requirement: Update agent model configuration
The system SHALL expose a write endpoint accepting a per-agent list of `{model, variant?}` entries for the agent's model chain. A write SHALL apply to the target agent's `agents.<name>` configuration in `~/.omo/omo.jsonc`, using schema-valid keys (`model` for a single primary, `models` for a chain), leaving every other key and agent unchanged, and SHALL preserve the file's `$schema` pin. After a successful write the system SHALL restart the ai-dev service. The Admin UI SHALL restrict the model choices offered for an agent to models present in the catalogs of currently connected providers; the write endpoint SHALL NOT apply that restriction (any syntactically valid model string is accepted).
...
- **THEN** only `agents.sisyphus` model configuration changes; `agents.prometheus` and the `$schema` value are byte-identical afterwards
