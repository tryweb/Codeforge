...
> **Scope.** This capability defines the shared wire protocol between the agent and the Center Server. Agent-side obligations (sending `hello`, correlating acks, reporting outcomes) are implemented by this change; Center Server obligations are specified here as the contract the future Center Server must implement.
