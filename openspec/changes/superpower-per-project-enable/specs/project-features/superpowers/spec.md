...
### Requirement: SuperPower installed globally in Docker image
**Reason**: Replaced by per-project opt-in model; global installation forces skills on all projects
**Migration**: Admins enable SuperPower per-project via admin UI after container deployment

### Requirement: SuperPower skills symlinked globally at container startup
**Reason**: Global symlinks bypass project isolation; skills should only be available where explicitly enabled
**Migration**: Per-project symlinks created on-demand when admin enables the feature