...
- **Live verification on save**: after restart, the UI re-queries the managed opencode server `/agent` endpoint (Basic auth `opencode:<OPENCODE_SERVER_PASSWORD>` from the mounted `.env`) and shows the new resolved model; on mismatch it rolls back the config change and reports the failure.
